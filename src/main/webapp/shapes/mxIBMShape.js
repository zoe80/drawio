...
mxIBMShapeBase.prototype.getProperties = function(shape, width, height)
{
	...
	{
		secondLine = false;
		if (shapeLayout === 'itemColor')
			fillColor = lineColor;  // Line fill color legend.
	}
...
mxIBMShapeBase.prototype.paintBadge = function(c)
{
	...
	if (pop.badgeVisible) {
		...
		if (badgeText != null)
		{
			...
			c.setFontFamily(ibmConfig.ibmFonts.sansregular);
...
mxIBMShapeUnit.prototype.paintIcon = function(c)
{
	...
	if (!pop.hideIcon)
	{
		let positionX = pop.iconSpacing;
		let positionY = pop.iconSpacing;
